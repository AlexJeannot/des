...
void        bytes_join(t_data *current_data, char *buf, u_int64_t buf_length)
{
    ...
    ft_bzero(new_content, (current_data->rc_size + buf_length));

    ft_memcpy(new_content, current_data->input, current_data->rc_size);
    ft_memcpy(&(new_content[current_data->rc_size]), buf, buf_length);
...
void        set_file_context(t_data *new_data, char *file_path)
{
    ...
    if (!(new_data->src = (char *)malloc(ft_strlen(file_path) + 1)))
        fatal_error("file source memory allocation");
    ft_bzero(new_data->src, (ft_strlen(file_path) + 1));

    ft_strncpy(new_data->src, file_path, ft_strlen(file_path));
}